...
def test_case_1_parse_dinner_message(sim: WhatsAppSimulator) -> None:
    ...
    now = datetime.now()
    parsed_date = parsed["reservation_at"].date()
    _assert(parsed_date in {now.date(), (now + timedelta(days=1)).date()}, "Case 1 unexpected parsed date")


def test_case_2_italian_food_search(sim: WhatsAppSimulator) -> None:
    result = sim.mock_incoming_user_message(
        from_number="whatsapp:+15550001111",
...
def test_case_3_yes_confirms_booking(sim: WhatsAppSimulator) -> None:
...
def test_case_4_invalid_inputs(sim: WhatsAppSimulator) -> None:
